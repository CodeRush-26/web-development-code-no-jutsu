import { Bell, HelpCircle, Monitor, ChevronDown, LogOut } from 'lucide-react';
  ...
  return (
    <header className="h-16 shrink-0 px-6 border-b border-bg-line bg-bg-panel/40 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-bg-card border border-bg-line text-sm">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-bg-card border border-bg-line">
          <span
            className={`w-2 h-2 rounded-full ${isConnected ? 'live-dot' : 'bg-accent-red'}`}
          />
...
function IconBtn({ icon: Icon, count }) {
  return (
    <div className="relative w-9 h-9 rounded-lg bg-bg-card border border-bg-line flex items-center justify-center text-ink-2">
